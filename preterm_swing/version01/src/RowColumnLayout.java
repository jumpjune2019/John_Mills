...
public class RowColumnLayout extends JPanel{
	...
	public RowColumnLayout(MyList myList){
	...
	private void prepareRowAndColumn() {
		...
		//creating new grid panel
		//and passing it a list so that
		//each button can update list scroll
		gridPanel = new JPanel(new GridLayout(10, 10));
		for(int i=0; i < 10; i++) {
			for(int j=0; j < 10; j++) {
				gridPanel.add(new MyCell(myList, i, j));
			}
		}//end outer for
